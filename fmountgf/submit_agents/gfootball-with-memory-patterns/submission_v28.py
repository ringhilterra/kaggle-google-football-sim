# start executing cells from here to rewrite submission_v28.py
...
# "%%writefile -a submission_v28.py" will append the code below to submission_v28.py,
# it WILL NOT rewrite submission_v28.py
...
def far_from_goal_high_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        if (obs["ball_owned_player"] == obs["active"] and
                obs["ball_owned_team"] == 0 and
                (player_x < -0.4 or obs["ball_owned_player"] == 0)):
            return True
        return False
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        biggest_distance, final_opponents_amount = get_average_distance_to_opponents(obs, player_x + 0.03, player_y)
        obs["memory_patterns"]["go_around_opponent"] = Action.Right
        # if top right direction is safest
        top_right, opponents_amount = get_average_distance_to_opponents(obs, player_x + 0.03, player_y - 0.02)
        if (top_right > biggest_distance and player_y > -0.15) or (top_right == 2 and player_y > 0.15):
        ...
        bottom_right, opponents_amount = get_average_distance_to_opponents(obs, player_x + 0.03, player_y + 0.02)
        if (bottom_right > biggest_distance and player_y < 0.15) or (bottom_right == 2 and player_y < -0.15):
            biggest_distance = bottom_right
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.BottomRight
        # if top direction is safest
        top, opponents_amount = get_average_distance_to_opponents(obs, player_x, player_y - 0.02)
        if top > biggest_distance and player_y > -0.15:
            biggest_distance = top
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.Top
        # if bottom direction is safest
        bottom, opponents_amount = get_average_distance_to_opponents(obs, player_x, player_y + 0.02)
        if bottom > biggest_distance and player_y < 0.15:
            biggest_distance = bottom
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.Bottom
        # if top left direction is safest
        top_left, opponents_amount = get_average_distance_to_opponents(obs, player_x - 0.03, player_y - 0.02)
        if top_left > biggest_distance and player_y > -0.15:
            biggest_distance = top_left
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.TopLeft
        # if bottom left direction is safest
        bottom_left, opponents_amount = get_average_distance_to_opponents(obs, player_x - 0.03, player_y + 0.02)
        if bottom_left > biggest_distance and player_y < 0.15:
            biggest_distance = bottom_left
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.BottomLeft
        # if left direction is safest
        left, opponents_amount = get_average_distance_to_opponents(obs, player_x - 0.03, player_y)
        if left > biggest_distance:
            biggest_distance = left
            final_opponents_amount = opponents_amount
            obs["memory_patterns"]["go_around_opponent"] = Action.Left
